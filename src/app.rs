...
fn doctor() -> Result<()> {
    ...
    println!("  scan:                no movement");
    println!("  plan:                no movement");
    println!("  organize:            no movement by itself");
    println!("  manifest:            no movement");
    println!("  preflight:           no movement");
    println!(
        "  apply:               GUARDED APPLY ONLY — requires manifest + preflight + backup + explicit flags"
    );
    println!("  rollback:            available from apply receipt");
    println!("  Real file movement:  GUARDED APPLY ONLY");
    println!("  Safe Autopilot:      plan eligibility only, does not move by itself");
    println!("  Guided Review:       question/review workflow only");
    println!();
    println!("  ─── Apply Safety Gates (all required for real moves) ───");
    println!("  1. Manifest must be a valid SafeSort plan (dry_run_only=true)");
    println!("  2. Preflight must pass all 8 checks");
    println!("  3. --backup flag required (freeze-state copy before every move)");
    println!("  4. --apply-safe-only flag required (only auto_plan_eligible entries)");
    println!("  5. --confirm flag required");
    println!("  6. --i-understand-this-moves-files flag required");
    println!("  7. Backup checksum verified before move");
    println!("  8. Destination checksum verified after move");
    println!("  LOCKED / REVIEW / MEDIUM / HIGH / CRITICAL items: never moved");
    println!();
    ...
    println!("  ✅ manifest      — SHA-256 checksum plan records (no movement)");
    println!("  ✅ preflight     — validates all safety gates (no movement)");
    println!("  ✅ apply         — guarded file movement (requires all safety flags)");
    println!("  ✅ apply-status  — read-only status of a previous apply run");
    println!("  ✅ rollback      — restore files from freeze-state backup (no new moves)");
